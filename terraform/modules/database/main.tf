...
resource "aws_db_instance" "runsabba_db_1" {
    ...
    parameter_group_name = "default.mysql5.7" #default config for mysql 5.7
    db_name = "runsabba_db_1"
    username = var.db_username
    password = var.db_password
    allow_major_version_upgrade = true #example: 5.7.44 to 8.0
    auto_minor_version_upgrade = true #example: 5.7.44 to 5.7.46
    backup_retention_period = 35 #automated backups saved for 35 days, older back ups will be deleted.
